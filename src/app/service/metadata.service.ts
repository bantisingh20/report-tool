...
export class MetadataService {
  ...
  getAvailableFieldsForTables(selectedTables: string[]): Observable<any[]> {
    return this.http
      .post<any>(`${API_URL}/check-table-relations`, {
        selectedTables,
      })
      .pipe(
        map((result: { columnsByTable: { [key: string]: any[] } }) => {
 ...
 getDataforPreviewPerPage({ page, pageSize, queryKey }: { page: number, pageSize: number, queryKey: string }) {
  return this.http.get(`${API_URL}/report/pagination`, {
    params: {
      page: page.toString(),
  ...
  processPreviewData(responseData: any) {
    ...
    const queryKey = responseData?.queryKey || null;
    console.log('service', rawData);
    console.log('service console', responseData);
